...
ggplot(car_dataset_total, aes(x = fuel, fill = fuel)) +
  geom_bar() +
  ggtitle("Distribución de Tipos de Combustible en car_data_2") +
...
ggplot(car_dataset_total, aes(x = transmission, fill = transmission)) +
  geom_bar() +
  ggtitle("Distribución de Tipos de Transmisión en car_data_2") +
...
ggplot(car_dataset_total, aes(x = year, y = selling_price)) +
  geom_point() +
  ggtitle("Relación entre Año y Precio de Venta en car_data_2") +
  xlab("Año") +
...
ggplot(car_dataset_total, aes(x = factor(1), fill = seller_type)) +
  ...
  coord_polar(theta = "y") +
  ggtitle("Distribución de Tipos de Vendedor en car_data_2") +
  theme_void() +
...
ggplot(car_dataset_total, aes(x = factor(owner), fill = owner)) +
  ...
  ggtitle("Distribución de Tipos de Propietario en car_data_2") +
...
ggplot(car_dataset_total, aes(x = transmission, y = selling_price, fill = transmission)) +
  ...
  ggtitle("Distribución de Precios de Venta por Tipo de Transmisión en car_data_2") +
  xlab("Tipo de Transmisión") +
  ylab("Precio de Venta") +
  theme_minimal()
```
...
train_data <- train_data[!(train_data$fuel %in% c("Petrol + CNG")), ]
test_data <- test_data[!(test_data$fuel %in% c("Petrol + LPG")), ]
...
model <- lm(selling_price ~ km_driven + year + fuel + transmission + owner + seller_type, data = train_data)
summary(model)
...
model_CV <- train(selling_price ~ km_driven + year + fuel + transmission + owner + seller_type,
...
ggplot(comparison_cv, aes(x = Real, y = Predicciones)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
  ggtitle("Comparación entre Valores Reales y Predicciones (Validación Cruzada k-fold)") +
  xlab("Precio Real") +
  ylab("Predicciones")

# Calcular el Error Cuadrático Medio (MSE) durante la validación cruzada
mse_cv <- mean((predictions_cv - car_dataset_total$selling_price)^2)
r_squared_cv <- cor(predictions_cv, car_dataset_total$selling_price)^2

cat("Error Cuadrático Medio (MSE) durante la Validación Cruzada:", mse_cv, "\n")
cat("Coeficiente de Determinación (R²) durante la Validación Cruzada:", r_squared_cv, "\n")
```
